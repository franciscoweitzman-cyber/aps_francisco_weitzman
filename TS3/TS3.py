...
tt=nn/fs
###########zpnn = np.zeros(9*N).reshape(1,9*N) + nn # Zero Padding
###########zptt=zpnn/fs
mu=0 #valor medio
...
eje_frecuencias = np.arange(N//2) * fs/N # debo poner 9*N?
...
# El problema está acá que llamo 3 veces a mi funcion sen
...
plt.title("Densidad Espectral de Frecuencia [dB]")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
pot_xx = np.mean(xx**2,axis=1) # Debiera ser sin dividir por N, pero al transformado que ya tiene 1/N lo vamos a elevar al cuadrado
